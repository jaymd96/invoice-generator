"""
Consolidated Timesheet Generator - browser-adapted version.
Day and period views with multiple header layouts. No file I/O.
"""
...
def _get_day_view_template(header_layout='default'):
...
def _get_period_view_template(header_layout='default'):
...
def _format_day_entries(entries, hide_task_hours=False):
...
def _format_period_entries(entries):
...
def _calculate_project_breakdown(entries):
...
def generate_timesheet(data, view='day', header_layout='default',
